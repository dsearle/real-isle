...
test("migrations 0010-0015 keep trigger SQL safe for the Sites BEGIN/END splitter", () => {
  ...
  assert.ok(triggerCount > 0, "expected trigger definitions in migrations 0010-0015");
});

test("a fresh database accepts migrations through 0015 after Sites-style trigger splitting", () => {
  const db = new DatabaseSync(":memory:");

  try {
    for (const file of migrationFiles().filter((file) => migrationNumber(file) <= 15)) {

...
BEGIN SELECT RAISE(ABORT, 'audit events are immutable'); END;--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS source_snapshots_no_update BEFORE UPDATE ON source_snapshots
BEGIN SELECT RAISE(ABORT, 'source snapshots are immutable'); END;--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS source_snapshots_no_delete BEFORE DELETE ON source_snapshots
BEGIN SELECT RAISE(ABORT, 'source snapshots are immutable'); END;--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS source_item_versions_no_update BEFORE UPDATE ON source_item_versions
BEGIN SELECT RAISE(ABORT, 'source item versions are immutable'); END;--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS source_item_versions_no_delete BEFORE DELETE ON source_item_versions
BEGIN SELECT RAISE(ABORT, 'source item versions are immutable'); END;--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS candidate_profile_observations_no_update BEFORE UPDATE ON candidate_profile_observations
BEGIN SELECT RAISE(ABORT, 'candidate profile observations are immutable'); END;--> statement-breakpoint
CREATE TRIGGER IF NOT EXISTS candidate_profile_observations_no_delete BEFORE DELETE ON candidate_profile_observations
BEGIN SELECT RAISE(ABORT, 'candidate profile observations are immutable'); END;--> statement-breakpoint
...
CREATE TRIGGER IF NOT EXISTS transcripts_no_delete BEFORE DELETE ON transcripts
...
CREATE TRIGGER IF NOT EXISTS transcript_segments_no_delete BEFORE DELETE ON transcript_segments
